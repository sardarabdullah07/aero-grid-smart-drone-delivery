...
> **Known issue:** `/setup` and the mission page both navigate to `/build`, but `frontend/src/app/build/` is not present in this repository, so those links currently 404. The likely cause was the `.gitignore`: an unanchored, Python-oriented `build/` rule also matched `frontend/src/app/build/` and silently excluded the city-builder page from version control. That rule has since been anchored to `/frontend/build/` and `/backend/build/`, so the route is no longer ignored — but the page source itself is absent and needs to be restored from a local copy or rewritten.
